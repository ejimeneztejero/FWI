...
if(rank.eq.0)    then
        write(*,*)
        write(*,*)'*************************************'
        write(*,*)'CHECK OBS DATA'
        write(*,*)'*************************************'
endif

!! FOR SHOTS-WAS
call check_nav_shot_was(rank) !ojo, perhaps ony rank=0. Intentar que solo haya warnings y errores
call read_nav_shot_was(rank)
!! FOR OBS
call check_nav_obs(rank) !ojo, perhaps ony rank=0. Intentar que solo haya warnings y errores
call read_nav_OBS(rank)

call open_obs(rank)     !!opens unit file for WAS data
call check_obs(rank)    !!check shotnumber content in SU file

if(rank.eq.0)   then
do iOBS=1,NumOBS
do i=1,NumShots_was
        if(shotID_su_was(i,iOBS).ne.shotID_nav_was(i))   then
...
if(rank.eq.0)    then
        write(*,*)
        write(*,*)'*************************************'
        write(*,*)'GEOMETRY FOR OBS DATA'
        write(*,*)'*************************************'
endif

!! GEOMETRY FOR INVERSION OF WAS DATA (OBSs ACT AS SOURCES, SHOTS AS RECEIVERS)
call get_geometry_WAS(rank) !! valores para nxSou_WAS,nySou_WAS,nxRec_WAS,nyRec_WAS
...
if(rank.eq.0)	then
        write(*,*)
        write(*,*)'*********************************'
      	write(*,*)'RELOCATION OF OBS '
        write(*,*)'*********************************'
endif

call relocation_zOBS(rank)

END subroutine get_was_data
...
do i=1,NumShots_was !!number of lines nav_shot
    ...
    pos_shot_was(i) = added_space_model+abs(xs)
    pos_shot_grid_was(i)=1+ceiling(pos_shot_was(i)/dmodel)

    pos_bat_was(i) = abs(bat)
    pos_bat_grid_was(i) = 1+ceiling(pos_bat_was(i)/dmodel)

    if(i.eq.1)xs1=xs
    if(i.eq.NumShots_was )xsn=xs

enddo

   dshots_was=abs(xsn-xs1)/(NumShots_was-1)
   length_model_was=xs+2*added_space_model
...
do i=1,NumOBS !!number of lines nav_obs
    ...
    pos_xobs(i) = added_space_model+abs(xs)
...
subroutine get_geometry_WAS(rank)
...
do irec=1,NumRec_WAS	!!receivers are the shots
	nxRec_WAS(irec,iOBS)=pos_shot_grid_was(irec)
	nyRec_WAS(irec,iOBS)=1+ceiling(shot_depth_was/dmodel)
enddo
...
subroutine get_obs_data(rank,iSS,NumSou_SS,NumRec_SS,Coding,raw_data)
...
IMPLICIT NONE
    ...
    integer(4) :: pos_r
    real    :: raw_data(nt_was,NumRec_SS)
    real    :: Coding(NumSou_SS)
    real(4), allocatable :: Data_tmp(:,:),sudata(:,:)

    nh = size_su_header

    allocate(Data_tmp(nt_was,NumRec_SS))
    allocate(sudata(nt_was+nh,NumRec_SS))
    Data_tmp=0.;sudata=0.;
    raw_Data=0.
...
!    write(*,*)'Summing up OBS num: ',iOBS,"with params ",iSS,k,NumSS_was

    do j=1,NumRec_SS
	pos_r = 1 + (j-1)*(nh+nt_was)*4
	READ(unit_was+iOBS, pos=pos_r)  sudata(1:nt_was+nh,j)
    enddo
    ...
    raw_data=raw_data+Coding(k)*Data_tmp
...
READ(unit_was+iOBS,pos=sizeof_was(iOBS)-(nh+nt_was)*4+pos_read) shotID_was_n
...
subroutine Ricker_was(rank,iSS,NumSou_SS,Coding,raw_Source)
...
implicit none
    ...
    integer :: i,j,k,rank,iOBS,iSS,NumSou_SS
!    integer :: SourceNum
    real    :: Coding(NumSou_SS)
    real    :: raw_Source(nt_was,NumSou_SS)
    real :: pi,f0,wc,t0,tmp,S0(nt_was),t(nt_was)
    real :: n
	...
	raw_Source=0
	do k=1,NumSou_SS
		!write(*,*)"Coding: ",Coding(k)
		do i=1,nt_was
		        raw_Source(i,k)=S0(i)*Coding(k)
		enddo
	enddo
...
!           write(18,*)i,xmodel(i),bat_model_was(i)
        ...
        do i=1,nxmodel
                bat_model_grid_was(i)=1+ceiling(bat_model_was(i)/dmodel)
        enddo
