...
!! iprop=3 Forward propagation test
...
subroutine solver_acoustic_SS(rank,iprop,NumSou,NumRec,nxSou,nySou,nxRec,nyRec,&
S,Sadj,d,dt,ny,nx,ctmp,&
store_snap,nt_Store,nt,kernel,Fields,Rec_point)
...
implicit none
	...
	real :: dt,d
	real :: S(nt,NumSou)
	real :: Sadj(nt,NumRec),Rec_point(nt,NumRec)
	real :: ctmp(ny,nx),kernel(ny,nx),kernel2(ny,nx)
	real :: Fields(ny,nx*(nt_Store+1))
	...
	character(len=100) :: file_name,file_name2,file_name3
	character(len=100) :: Strtmp,Strtmp2

	dPML=20
	acc=3
	Rec_point=0.
	free_surface=1	!! si =0, PML absorbente
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	if(iprop.eq.2)	then	!! problema para SS. OJO
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	do k=2,nt
		...
		if(iprop.ne.2)	then
	        	do j=1,NumSou
		!		write(112,*)j,nys(j),nxs(j),S(k-1,j)
...
!!! 		Field and Traces saving
		if(iprop.eq.1)	then

			if (l.eq.store_snap) then	

			snap=snap+1
			if(snap.gt.nt_Store)write(*,*)'nap overflow 1',snap

			do i=1,nx
			do j=1,ny
				Fields(j,snap*nx+i)=(p_3(dny2+j,dnx+i)-p_1(dny2+j,dnx+i))*0.5/dt
			enddo
			enddo
		...
		if(iprop.eq.2)	then
			...
			if(l.eq.store_snap) then
			snap=snap-1
			if(snap.gt.nt_Store)write(*,*)'snap overflow 2',snap

			do i=1,nx
			do j=1,ny
				kernel2(j,i)=-0.5*(p_3(dny2+j,dnx+i)-p_1(dny2+j,dnx+i))*Fields(j,snap*nx+i)*store_snap
				kernel(j,i)=kernel(j,i)+kernel2(j,i)
			enddo
			enddo
		...
		if(iprop.ne.2)	then			
			do i=1,NumRec
				Rec_point(k-1,i)=p_3(nyreco(i),nxreco(i))!!ojo, falta NumSou, coding gather?
		        enddo
		endif
	


               if(rank.eq.1939390.and.MOD(k,nn).eq.0)        then
!                if(rank.eq.0.and.k.eq.nt/2.and.iprop.eq.1) then

                        icont=icont+1
                        write(Strtmp2,*) icont
!                       write(*,*) icont*25,'% solver done'

!                       if(iprop.eq.1)file_name=trim('propF_'//adjustl(Strtmp2))//'.txt'        
!                       if(iprop.eq.2)file_name=trim('propB_'//adjustl(Strtmp2))//'.txt'
!       

                        open(unit=12,file="Prop1_ss.dat",status='unknown')
                        do j=1+dny2,dny2+ny
                                write(12,'(20000(e12.5,2x))') (p_3(j,i),i=1+dnx,dnx+nx)
                        enddo
                        close(12)

!                       if(iprop.eq.1)file_name=trim('sourceF_'//adjustl(Strtmp2))//'.txt'      
!                       open(unit=12,file=file_name,status='unknown')   
!                       do j=1+dny2,dny2+ny
!                               write(12,'(20000(e12.5,2x))') (source(j,i),i=1+dnx,dnx+nx)
!                       enddo
!                       close(12)
	...
	Rec_point(nt,:)=Rec_point(nt-1,:)

!        open(unit=12,file="data_synth2.dat",status='unknown')   
!        do j=1,nt
!        write(12,'(20000(e12.5,2x))') (Rec_point(j,i),i=1,NumRec)
!        enddo
!        close(12)
...
end subroutine solver_acoustic_SS	
